...
test("head progress survives a callback crash, resumes receipts without source refetch, and rolls back an unrecorded resolution", async context => {
  ...
  try {
    ...
    assert.equal((proof?.receipt?.details as { batchNumber?: number } | undefined)?.batchNumber, 1);
    ...
    const terminalReceipt = await client.local_audit_events.findFirstOrThrow({ where: {
      action: PROVIDER_HEAD_RECONCILIATION_ACTION, target_id: child.id,
    }, orderBy: { sequence: "desc" } });
    assert.equal((terminalReceipt.details as { batchNumber?: number }).batchNumber, 2);
