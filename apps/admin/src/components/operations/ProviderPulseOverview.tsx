...
import { count, measurementTotal, metricDescriptions, pulseNeedsAttention, pulseState, sortPulseSources, storedCount } from "./provider-pulse-presentation";
  ...
  return (
    <section className="provider-pulse" aria-label="Provider overview">
      <dl className="provider-pulse__summary" aria-label="Pipeline totals">
        ...
        <div><dt><IndicatorTooltip label="Stored rows" description={metricDescriptions.stored} /></dt><dd>{storedCount(stored.value, stored.estimated)}<span className="provider-pulse__subtext">{stored.estimated ? `${stored.coverage} · estimated` : stored.coverage}</span></dd></div>
