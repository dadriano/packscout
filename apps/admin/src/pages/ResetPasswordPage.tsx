 ...
 * are the same ones the rest of the admin shows.
 */
...
function InvalidLinkState() {
  return (
    <>
      ...
      <p role="status">
        Reset links work once and expire soon after they are sent, and
...
export function ResetPasswordPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    // The same schema — and therefore the same messages — the completion
    // endpoint and the administrator-facing credential forms enforce.
    const parsed = passwordResetCompletionRequestSchema.safeParse({
      token,
      password,
    });
    if (!parsed.success) {
    ...
    try {
      ...
      if (caught instanceof AdminApiError) {
        ...
        if (caught.code === "VALIDATION_FAILED") {
          setError(passwordMessageFrom(caught) ?? "Check the new password and try again.");
          return;
        }
      }
