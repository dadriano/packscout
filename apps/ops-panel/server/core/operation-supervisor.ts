 ...
 *     against each other. It gates nothing else — log tailing, status reads and
 *     the row browser are unaffected — so the panel stays usable while a
 *     migration runs.
 ...
 *  4. **An interrupted run is not lost.** A durable marker is written before the
 *     child starts and cleared only when the run settles, so a panel that is
 *     killed mid-operation reports that operation's outcome as *unknown* on its
 *     next start instead of forgetting it happened.
 *
...
export interface DatabaseOperationRunner {
  ...
  start(request: OperationStartRequest): OperationStartResult;
...
export function createDatabaseOperationRunner({
  ...
  function persist(marker: OperationMarker | null): void {
    void markerStore.save(marker).catch((error: unknown) => onPersistenceError?.(error));
  }
  ...
  function settle(run: ActiveRun, outcome: OperationOutcome, message: string): void {
    ...
    // as unknown after a restart is cleared.
    persist(null);
    emit({ type: "state" });
    onSettled?.(snapshot);
  }
  ...
  return {
    ...
    start(request) {
      ...
      active = run;
      lastOutput = [];
      // Written *before* the child starts: a crash between here and the first
      // line of output must still be reported as unknown rather than lost.
      persist({
      ...
      try {
        ...
        // adopting its handle now would leak it.
