...
test("accepts one strict reactive-read failure without catalog query data", async () => {
  ...
  assert.doesNotMatch(
    JSON.stringify(seen),
    /cursor|fingerprint|publicRepackId|publicCollectibleId|vendorKey/,
  );
});
...
test("rejects raw query, cursor, subject, and arbitrary metadata fields", async () => {
  for (const forbidden of [
    ...
    { publicRepackId: "beab33e4-20cf-5c41-9d31-2e616a34c113" },
    { publicCollectibleId: "beab33e4-20cf-5c41-9d31-2e616a34c113" },
...
test("rejects query/error combinations that cannot occur", async () => {
  const invalid = [
    ...
    { queryName: "getDashboardBundle", errorCode: "REPACK_NOT_FOUND" },
    { queryName: "getPublicRepack", errorCode: "CURSOR_EXPIRED" },
  ] as const;
...
test("requires active or safe-previous release context", async () => {
