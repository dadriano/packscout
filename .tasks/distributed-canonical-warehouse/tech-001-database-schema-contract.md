...
| `pulls` | `id uuid PK`, `pull_key text`, `fact_digest char(64)`, `pack_key text?`, `pack_id uuid?`, `provider_account_id uuid? FK`, `item_count int`, `occurred_at ts`, `paid_amount numeric(38,18)?`, `paid_currency varchar(42)?`, `row_version bigint=1`, `created_at ts`, `updated_at ts` | UK `pull_key`; optional composite `(pack_id,pack_key)` FK; digest covers immutable source keys, the positive immutable expected item count, and ordered item facts; null key means source-unreported, nonnull key/null ID means unresolved; only null-to-matching-ID resolution may update; each insert/resolution emits its exact promotion version; nonnegative money pair; resolved pack/time, unresolved-key, and account/time indexes |
| `pull_items` | `id uuid PK`, `pull_id uuid FK`, `ordinal int`, `collectible_key text?`, `collectible_id uuid?`, `collectible_instance_id uuid?`, `quantity bigint`, `stated_value_amount numeric(38,18)?`, `stated_value_currency varchar(42)?`, `row_version bigint=1`, `created_at ts`, `updated_at ts` | UK `(pull_id,ordinal)`; optional composite `(collectible_id,collectible_key)` FK plus instance/collectible FK; null key is an explicitly unreported outcome while keyed/null ID is unresolved; a supplied instance key is accepted only when its collectible and instance both resolve because instance source-key persistence/reconciliation is not yet designed; positive ordinal/quantity; only null-to-matching-ID resolution may update and increments promotion-visible row version |
| `market_events` | `id uuid PK`, `event_key text`, `fact_digest char(64)`, `event_group_id uuid?`, `event_type market_event_type`, `pack_key text?`, `pack_id uuid?`, `collectible_key text?`, `collectible_id uuid?`, `collectible_instance_id uuid?`, `from_provider_account_id uuid? FK`, `to_provider_account_id uuid? FK`, `quantity bigint?`, `occurred_at ts`, `amount numeric(38,18)?`, `currency varchar(42)?`, `details jsonb={}`, `row_version bigint=1`, `created_at ts`, `updated_at ts` | UK `event_key`; optional composite key/ID subject FKs; same key/digest replays and changed digest conflicts; at least one source subject key; keyed/null ID subjects are valid unresolved facts; a supplied instance key is accepted only when its collectible and instance both resolve; only null-to-matching-ID resolution may update and increments promotion-visible row version; quantity/money checks; group/type/time, resolved subject/time, and unresolved-key indexes |

A deferred constraint trigger rejects a committed `pulls` row unless its positive immutable `item_count` exactly matches the number of `pull_items` rows, sealing the completed pull against later item appends. A null item key represents one source-reported outcome whose collectible identity was unreported; it is not a fabricated catalog entity. Immutable source keys are the durable correlation authority, while nullable local UUIDs are one-way resolution state. A bounded reconciler resolves keyed rows after page commits and drains at source head without rolling back an already committed source cursor. Compound transfers or swaps use multiple atomic market rows with one deterministic `event_group_id`; individual amounts and directions remain on each row.
...
| `quarantine_records` | `id uuid PK`, `provider_run_id uuid FK`, `provider_run_page_id uuid`, `record_index int`, `record_kind text`, `entity_key text?`, `source_record_key text?`, `external_id text?`, `reason_code text`, `field_path text?`, `sanitized_summary text`, `candidate_schema_version text`, `normalized_candidate jsonb?`, `protected_evidence jsonb?`, `evidence_expires_at ts`, `evidence_expired_at ts?`, `retry_count int=0`, `last_retry_at ts?`, `resolved_at ts?`, `state quarantine_state`, `row_version bigint=1`, `created_at ts`, `updated_at ts` | Composite FK `(provider_run_page_id,provider_run_id)` prevents mismatched ownership; UK `(provider_run_page_id,record_kind,record_index)` and nullable UK `source_record_key`; nonnegative index/retries; normalized-candidate commit failures start `open` with the default 90-day retry window; source/adapter/mapper rejections use a deterministic provider-plus-stream/entity source identity (falling back to the individual record digest when identity is absent), retain neither candidate nor evidence, and are inserted terminal `expired` with both evidence times at page commit; redelivery of the same source quarantine is a duplicate with no new row/outbox, and no source quarantine exposes a retry action; tombstone candidate/evidence, never history |
...
Quarantine retry consumes the stored provider-neutral `normalized_candidate`; it never refetches or remaps source data. Only an `open` normalized-candidate commit failure is retryable. A strict source/adapter/mapper rejection has no retained retry artifact, is terminal `expired` at insertion, and carries explicit non-retryable admin/activity copy. A successful retry atomically writes the exact canonical tables above, its promotion changes, attempt outcome, quarantine resolution, and local audit without changing the original run/page counters or source cursor.
...
4. Provider pulls contain one or more ordered items and may carry a source-unreported or keyed-unresolved pack/card relationship; market events retain source subject keys and optional resolved local UUIDs/account parties.
...
| Category/alias cycle, instance mismatch, empty pull, missing all market source subjects, illegal relationship relink, or invalid money pair | Reject the local transaction with no cursor, checkpoint, or partial canonical effect |
| Valid source fact references a catalog row that is absent or arrives later | Store immutable source keys with null local UUIDs, commit the page/cursor, and reconcile in bounded promotion-visible batches |
...
- Verify row-version no-ops, retirement, immutable fact payloads, one-way relationship resolution, and restricted deletion under the real application roles.
