...
Every buyer-facing EV surface communicates last-known age, decayed confidence, and provider ranking eligibility consistently without treating age as missing data.
...
- Exclude only provider-ineligible rows from Top Opportunities and explain the reason separately.
- Update overview coverage, glossary, and Learn copy to say age alone never makes a calculable estimate unavailable.

## User-Facing Behavior

Rows, cards, inspectors, summaries, and education use these exact concepts: `Last-known estimate`, `Source evidence last observed …`, `Provider feed delayed; excluded from Top Opportunities.`, and `Unavailable means required supported inputs are missing. Age alone does not make an estimate unavailable.`

## Interface Contract

The browser consumes presentation state, server-derived confidence, source time, the separately server-derived confidence and provider-health evaluation times, limitations, provider health, and ranking eligibility. It formats values, schedules a refresh from the trusted provider-health clock pair, and renders accessible explanations but never recalculates policy.

## Acceptance Criteria

- [x] Current and last-known estimates display all four EV metrics across table, cards, inspector, and overview.
- [x] Healthy old estimates may rank; delayed-provider old estimates stay visible but do not appear in Top Opportunities.
...
The frontend consumes only server-derived presentation and provider-health state. It does not recalculate age, confidence, availability, or Top Opportunities eligibility in browser code. Known last-known EV remains visible and sortable, while provider delay is disclosed and gates only Top Opportunities.
