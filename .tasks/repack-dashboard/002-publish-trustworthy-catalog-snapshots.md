...
- Implemented: strict browser-safe `CatalogSnapshotV1`/public DTO schemas with required `dataSource: "canonical" | "mock"`; protected-field scanning; exact materialized metric validation; approved-host/action validation; and fresh/delayed/unavailable read-state contracts. A deterministic internal Convex mutation provides a guarded development mock lane with one complete snapshot, 9 packs, one query shard, singleton state, and one publication-operation receipt; it refuses disabled, production, canonical-active, conflicting, and partial targets before replacing data.
- Verified: contracts tests prove protected fields are rejected, sold-out Pack Links are invalid, money/basis-point consistency is exact, public reason codes are bounded, and action/media hosts must match the copied approved configuration. Local seed tests and cloud-development command evidence against `abundant-puffin-373` prove exact 9-pack creation, one operation receipt, and idempotent `created` then `unchanged` outcomes with no duplicate rows.
- Blocked: task `001` has not produced an adopted V2 canonical handoff. There is no dual-approved PostgreSQL public configuration, publication ledger/manifest-block authority, HMAC key set, authenticated publisher, canonical staging/activation lifecycle, recovery path, or real publication evidence. The cloud development deployment currently contains explicitly labeled mock data only.
...
- Divergences: the internal development mock seed is not a substitute for the canonical PostgreSQL ledger, authenticated publisher, staged activation, reconciliation, rollback, retention, or admin workflow.
- Verification: `@packscout/contracts` snapshot/public-catalog tests, local Convex/read-model suites, and local plus `abundant-puffin-373` seed create/replay proof recorded green; no canonical publication or real-backend evidence exists.
