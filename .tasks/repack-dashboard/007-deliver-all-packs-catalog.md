...
- Implemented: canonical URL-backed search/facets/price/sort/cursor state, the exact twelve-column All Packs table, relevance mode, unavailable-last sort affordances, result range/pagination, selected bottom preview, and narrow-screen inspector sheet under `apps/frontend`.
- Verified: public read-model tests cover filtering, relevance, contextual facets, sorting, price semantics, and cursors; frontend tests cover exact columns, sort state, sold-out actions, URL normalization/history state, and first-page resets; browser review confirms internal table overflow with no page-level overflow.
- Blocked: task `003` has no activated/reactive cloud snapshot. Nested action versus row-selection behavior and the live cursor/history flow still require recorded end-to-end evidence against the cloud read model.
...
- Divergences: initial/replacement reads use the temporary non-reactive route boundary from task `003`; live `usePreloadedQuery` replacement and cloud-backed cursor continuity remain open.
- Verification: contracts/Convex query tests, All Packs/query-state tests, frontend typecheck/lint/build, and desktop/mobile browser overflow and inspector review recorded green.
