...
P05/task 005 is merged in PR108 (`27c7f7ec`, 2026-09-04). P02 is rebased onto that main and passes 106 focused checks plus 30 schema checks, including the readiness-time transition and lifecycle `dataAsOf` regressions. Separate PR109 now passes the unchanged full local framework gate on `f8110b34`, including audit, tooling, and both builds; CI is pending. Its Watchlist response rename needs coordinated frontend/Convex release approval: main merges automatically deploy the frontend, while a read-only check confirms live Convex still returns `estimatedEv`. Do not merge the rename blindly. Once that prerequisite is safely merged, refresh/reverify and merge PR95, then restack and certify P03 separately. No publication processor is enabled.
