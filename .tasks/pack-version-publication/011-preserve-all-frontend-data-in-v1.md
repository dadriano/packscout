...
Superseding capacity question, September5 at21:42UTC: retain8,000 packs and ask only whether complete category filters keep the frontend's existing512-category bound or require a larger category catalog. The earlier1,000-versus8,000 question is withdrawn: the single-query full-rich-row failure does not require reducing pack capacity. No category reduction or changed facet-delivery contract is approved pending the answer. Continue003/004 verification independently; full contents remain8,000 members per individual pack.

Read-only thin-selection receipt: actual IDs are pps_+64 hex andppfs_+64 hex, not UUIDs. A candidate394-byte pack row plus455-byte referenced-provider head yields6,792,060 bytes for8,000+8,000 rows under installedgetConvexSize, with two index ranges and16,000 documents before auth/validation overhead. A hasPackHead marker/index on provider heads could bound the coherent provider selection without8,000 point reads or a new global provider cap; set it on first pack activation and preserve it on profile replacement. These are proposed internal derived indexes, not implemented source or live transaction/heap evidence.

Existing public actions can select pack/provider pointers coherently once, then hydrate only pinned immutable data in bounded batches with one calculation clock. Never reread current heads or substitute newer snapshots during hydration. Exact desired-membership lookup can use selected snapshot identity and staging-known content batch ordinal, avoiding unlimited retained-candidate scans. Displaced roots can become blocked before hydration despite30-day deletion retention; define bounded retry/unavailable behavior without partial exact counts. Conservative full-root hydration proves boundedness but may require2,000 batches, not acceptable latency: prefer and measure a compact immutable filter projection plus returned-page rich hydration. Stream exact predicates, medians and contextual facets; prove64MiB action heap,16MiB returns, transaction CPU/bytes/ranges and all normal/replacement/rollback writes. This feasibility work preserves8k as a target, not a certification claim. The only identified product/domain choice is category capacity; implementation and final performance proof remain.

Maximum-query audit, September5: installed `convex/values.getConvexSize` measures a schema-valid search/category projection at6,891 bytes per pack (1,024 derived UTF-16 units plus100 category UUIDs), or55,128,000 bytes for8,000 packs before identities/metrics/system fields/hydration. Even ASCII is38,856,000 bytes; IDs alone30,528,000; allowed Unicode text alone24,616,000. This proves a literal compact-row full scan cannot meet16MiB; these are payload lower bounds, not live transaction measurements. Probe receiptbc5871. Existing V3 actually declares1,000 packs/512 categories, while native V1 declares8,000 packs and no global category-universe ceiling. Do not confuse8,000 content members with proven catalog-query capacity or silently reduce any accepted bound. Complete facets also need an explicit bounded domain;100 categories per pack alone permits800,000 distinct IDs.

Preserve actual search semantics: AND-of-token-prefix over normalized name/vendor/category; NFKD/mark removal/dedup; nonempty search relevance outranks selected numeric sort. Current NFKC title+aliases substring matching is not parity. Category/type OR selections compose across dimensions; contextual facets ignore only their own selection, avoiding overlap double-counts. Confidence changes at evaluation time without publication, so publication-time confidence aggregates alone are insufficient. Aggregate component calls share the same transaction byte/range budget, and ordered-range aggregates do not automatically solve these intersections. Before fan-out, root must prove a lossless snapshot-bound membership/facet representation and combined normal/replacement/rollback/retained-candidate budget; no new architecture, public API, truncation or feature reduction was approved by this read-only audit. Sources: [Convex limits](https://docs.convex.dev/production/state/limits), [transaction accounting](https://docs.convex.dev/database/writing-data#measuring-document-sizes), [search semantics](https://docs.convex.dev/search/text-search), [Aggregate](https://github.com/get-convex/aggregate).

Current boundary: task013/P05B's neutral calculation/public-value extraction is merged through PR121, and the native frontend foundation012 is merged through PR120. Main advanced through unrelated PR122 to b96b43691b48170d64bdd541e4a42cb15f096ecf; preserve its user-selected/reordered All Repacks columns and account preference persistence in011/007 as well. P05A still waits for003/004's current review fixes to be verified and merged;005/013 already belong to main. Its two incomplete native economics files remain preserved, not completed. Recreate P05A from the merged prerequisites; the old directory/dependencies moved safely to P05B. All current frontend datapoints and unchanged formulas remain required. Older path/parent notes below are historical preservation evidence, not a request to recreate the already-completed013 extraction.
...
### Final field-boundary audit before implementation

Read-only audit against main b96b4369 confirms the contract work common to either catalog-capacity choice. Preserve nullable image plus supplied alt, full categories/types/content mode, nullable display price and discriminated USD comparison. Current collectible names and aliases permit240 characters; native200-character names/120-character aliases must not reject otherwise-valid existing data. Economics belongs to the sealed pack, including all four signed/unsigned metrics, retained calculation price, confidence/limitations/policy, source times, latest-unavailable reason and sellout evidence. Keep provider-reported values distinct from provider-derived metrics and independent EV. Shared collectible profiles need descriptors, category hierarchy, value evidence, aliases and subject search input; snapshot-pinned members need their actual display and chase/match evidence. Provider profiles own vendor identity, assets/promotions and sanitized health evidence; query response clocks remain explicit.

Reuse task013's `calculatePackScoutBuybackAdjustedEvV1`, `evaluatePackScoutBuybackEvConfidenceV1`, `packScoutBuybackEvMetricsAreConsistentV1`, `packScoutPublicEvMetricsV1Schema`, `publicBuybackSummaryV1Schema` and `vendorReportedEvV1Schema` unchanged. Existing provider-derived arithmetic and last-known presentation still require exact-body neutral extraction, not a second formula or a V3 DTO import. `public-ev-presentation-v1.ts` is not neutral merely by filename: it imports a V3 policy and includes RELEASE_MISMATCH. Preserve the underlying provider-health datapoints through a native closure, not an old-release adapter. Typed promotion codes remain1–64 ASCII letters/digits/period/underscore/hyphen with an alphanumeric start; free-form promo copy is not a code. Preserve all15 PR122 column keys/preferences, exact selected collectible even with zero matches, sealed match evidence, full query aggregates/facets and removable owner-unavailable rows. This is an implementation map, not completed code or an approved capacity change.
