...
The unrelated Watchlist gate is being repaired separately in [PR109](https://github.com/dadriano/packscout/pull/109), `.worktrees/fix-watchlist-ev-vocabulary`, branch `codex/fix-watchlist-ev-vocabulary`; its response uses `displayedEv` with a strengthened vocabulary guard. Do not add an inventory exemption. P03 remains local and must be restacked/certified separately after P02 is merged. Earlier blocker notes below are historical context until final evidence replaces them.
