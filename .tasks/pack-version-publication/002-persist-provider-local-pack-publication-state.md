...
The user authorized unblocking 002/003 and resumed after P05 merged. The existing P02 branch is now rebased onto main `27c7f7ec` (PR108), with its previously published `9b3fb05a` preserved at `codex/p02-before-p05-merge-20260904`. Both P1 crash/partial-expiry cases and the two later readiness-time/lifecycle timestamp findings have direct regressions. All 106 focused checks and 30 schema checks pass, but full certification and review resolution remain pending.

Episodes with unresolved operations or successful activation evidence survive enqueue, lease takeover, defer, and retry exhaustion even if no ambiguity marker was written. A blocked unresolved episode pauses only its pack. One shared receipt predicate distinguishes those hazards from complete non-activation evidence, so a definitive refusal does not strand corrected source input behind blocked history. `retireReconciled` requires the current lease, an expired or superseded desired episode, and a receipt for every recorded operation; successful/unknown activation or an expired activation replay cannot authorize replacement. A definitive activation refusal/conflict can. Retirement preserves all operation/receipt bytes, replans only the still-latest expired capture, and rechecks EV readiness before artifact reuse. P06 supplies authenticated receipts/status evidence; no transport or processor is enabled here.

The separate prerequisite [PR109](https://github.com/dadriano/packscout/pull/109), `.worktrees/fix-watchlist-ev-vocabulary`, branch `codex/fix-watchlist-ev-vocabulary`, repairs the Watchlist vocabulary guard and two PR105 test-fixture loops without changing resident runtime. After PR106/107 merged, its frontend response consumer was aligned too. The unchanged full framework gate passed on committed/pushed `f8110b34`, including audit, all tests/tooling, and both builds. CI is pending. A read-only function-spec check confirms live `shiny-newt-310` still returns `estimatedEv`, while main merges automatically deploy the frontend. Obtain coordinated frontend/Convex release approval before merging this response rename; do not add an adapter or assume backend deployment. P03 remains local and must be restacked/certified separately after the prerequisite and P02 merge.
...
PR96 merged at `1e79ff9ca961b569ca4b191e617b60dc315cc390` after its complete CI gate passed on `744bed7e` (run `33827531502`, retry two). PR95 is now based on main `27c7f7ec894996747095aa97652cd95aaefdc4e3`, including P05/PR108. Current runtime checkpoint `2b9874d1a1b84b9a470733a753da5020ed80021e` retains the rebased crash/expiry repairs and adds time-dependent readiness and lifecycle timestamp protection; subsequent handoff commits change documentation only. P03 remains a separate local branch at `c5329608` on old P02 boundary `8409143c`.

Full certification still requires the separately released PR109 prerequisite, then a P02 rebase and unchanged full gate on its final committed head. PR109 fixes the Watchlist response and merged frontend consumer plus PR105's test-only module-identity/completed-head fixtures; its full local gate passes, with CI/release approval pending. All four review findings now have implementation/evidence replies but remain open pending P02's full certification; the new replies are `3933884708` and `3933884948`. No publication processor or ingestion cutover is enabled, and no merge or deployment occurred during this repair.
