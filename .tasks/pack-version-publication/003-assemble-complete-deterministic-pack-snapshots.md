...
Policy pause: removing the Bearer minimum rejects valid existing prose controls, including a title `Bearer` with alias `Scout pack` after search normalization, and names such as `Bearer of the Heavens`. The assignment/short-token proposal remains uncommitted in the two owning guard/test files. Do not flip benign-success assertions to failures merely to satisfy review; root is asking for contextual recognition versus blanket fail-closed ambiguity. Latest published runtime remains c5c6782e/head8d723c2a. This is a product-policy choice, not a missing parser implementation or permission to relax public-output guards.
