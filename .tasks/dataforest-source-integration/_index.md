...
The stable business objects are the four PackScout platforms/providers. Each provider has its own replaceable source instance, opaque checkpoint, schedule, runs, health, and processor log feed. The four first-pass source instances share one DataForrest connection profile and one DataForrest adapter. DataForrest is an implementation of the provider-source boundary, not the boundary itself.

The first pass runs locally through one long-lived supervisor process. It owns four concurrent platform lanes, while PostgreSQL remains authoritative for schedules, queued runs, leases, checkpoints, progress, and restart recovery. The admin console configures and monitors those lanes; it does not run imports in the browser or create a second scheduler.
...
|                                                                    |
|  Per source: adapter revision/config, opaque checkpoint, schedule, |
...
+---------------------------------+----------------------------------+
   ...
   records + opaque checkpoint + continue/poll-after + safe evidence
                                  |
                                  v
              provider mapper --> generic atomic importer
                                  |
                                  v
               canonical data + EV work + checkpoint
```

A provider identity never contains a source-instance, source-adapter, or connection-profile identity. Replacing `courtyard-dataforest` with a future `courtyard-packscout-collector` creates a new source with its own checkpoint and changes only Courtyard's active source binding. The replacement must declare the same Courtyard identity-namespace key and emit stable IDs from that namespace; an incompatible source cannot activate without a separately designed identity-migration feature.
...
- Each provider owns one active source instance with an immutable platform filter and independent opaque checkpoint, 60-second default interval, run history, health, and processor log feed.
...
- Every adapter returns the same normalized provider-observation page: stable provider and record identities plus evidence-backed record-ID scope, ordered record outcomes, an adapter-owned opaque checkpoint, either `{ kind: continue }` or `{ kind: poll_after, minimumDelaySeconds }`, bounded measurements, and sanitized diagnostics.
...
- Every source instance owns its `sourceTypeKey`, adapter and normalized-contract revisions, mapper key and version, identity namespace, connection-profile revision, source configuration revision, checkpoint codec, opaque checkpoint value, and checkpoint generation.
- Checkpoints never move between source instances or adapter types; a replacement source starts with its own null checkpoint and establishes its own continuation state independently.
...
- One versioned provider-source contract is the sole authority for adapter capabilities, normalized observations, opaque checkpoints, continuation, identity, lifecycle, failure, and diagnostic semantics.
...
- One page may contain catalog, pulls, and trades for one platform; its opaque checkpoint belongs only to that source instance and adapter revision.
...
- Worker restart, timeout, pause, retry, and disable resume only from a committed opaque checkpoint; no failure silently restarts a source from the beginning.
...
- Processor feeds retain 30 days of sanitized page/run events. They never contain credentials, payloads, personal identifiers, full checkpoints or vendor cursors, authorization headers, or raw upstream bodies.
...
- Preserve canonical history, compact record dispositions, hashes, relationships, EV work, checkpoints, run summaries, health, and operator audits durably.
...
- A provider detail presents current source configuration, run history, safe checkpoint fingerprints, page progress, retry state, and its isolated processor log feed.
...
- Production and admin registries expose only `dataforrest-events-v1`; dynamic plugins, checkpoint translation, dual-source cutover, and incompatible-ID reconciliation remain out of scope.

## First-Pass Success

- All four platform processors can run concurrently and independently from the same local supervisor without sharing checkpoints, leases, log events, or failure state.
- Each provider completes its full historical backfill, reaches head, survives a restart from its committed checkpoint, and returns to its independently configured incremental schedule.
