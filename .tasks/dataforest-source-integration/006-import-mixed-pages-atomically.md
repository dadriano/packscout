...
Import one sanitized normalized Courtyard page and force persistence to fail immediately before checkpoint advancement; the first expected result is no partial canonical data, EV request, page diagnostic, or checkpoint change.

## Objective

Commit every normalized source page exactly once in effect so protected evidence, source lineage, valid observations, quarantined siblings, canonical changes, EV work, processor progress, normalized continuation, and the next opaque checkpoint cross one durable boundary together.
...
- Bind provider, source instance and revision, `sourceTypeKey`, source-adapter and normalized-contract revisions, mapper key and version, identity namespace, durable request-attempt identity and terminal capture outcome, request-time supervisor fencing epoch, connection profile and revision, request-time connection-health generation, checkpoint codec and generation, requested checkpoint, page, normalized continuation, and next checkpoint to one commit.
- Commit the authoritative protected raw-page evidence, stable source-record keys, semantic observations keyed by effective source time and versioned normalized-content hash, page-and-record-index delivery occurrences with complete lineage and dispositions, canonical revisions, relationships, counters, page progress, normalized continuation, and next opaque checkpoint atomically.
- Advance only the importing source's checkpoint and only after every normalized observation has a durable inserted, revised, duplicate, or quarantined disposition.
- Write one sanitized `PAGE_COMMITTED` processor diagnostic in the same transaction with page number, counts, timings, bytes, continuation kind, a required minimum delay only for `poll_after`, and safe checkpoint fingerprints.
- Leave page, canonical, EV, diagnostic, and checkpoint state unchanged when validation, planning, mapping, persistence, or diagnostic persistence fails before commit.
...
- Reproject retained quarantine evidence without rewinding the source checkpoint or replaying unrelated records.
- Reconcile unresolved pack and card relationships idempotently when their catalog dependencies arrive.

### Idempotency and checkpoint safety
...
- In the same atomic precondition, require the matching request attempt's terminal successful-capture outcome and require its request-time supervisor epoch to remain the current `active` owner; reject a nonterminal or mismatched attempt, released, superseded, expired, or `fenced_draining` epoch, stale run lease, disabled or replaced source, revoked credential revision, stale connection-health generation or open blocking connection episode, and old checkpoint generation before commit.
- Retry transient fetch failures against the same requested checkpoint and never restart from null after an action-required failure.

### Generic continuation and checkpoint guard

- Accept only `{ kind: continue }` with no delay and a nonnull next checkpoint or `{ kind: poll_after, minimumDelaySeconds }` with a required integer from 0 through 86,400; reject every other shape before persistence.
- For `continue`, fingerprint the next checkpoint and reject it if that fingerprint was previously committed for the same source and checkpoint generation, including A-to-B-to-A across runs or restarts.
- Commit the accepted next-checkpoint fingerprint and continuation atomically with the page so concurrent or replayed work cannot bypass cycle detection.
- Allow `poll_after` to preserve its requested checkpoint, including null, and use its required delay only for scheduling after commit.
- Never parse, convert, or transfer checkpoint bytes in the importer; adapter grammar and immediate requested-versus-next validation remain task 003's responsibility.
...
- Commit the EV request in the same transaction as the canonical revision, record disposition, page outcome, and checkpoint.
- Enqueue nothing for duplicate pages, collection-only replay, unchanged canonical content, or incomplete EV evidence.
- Roll back the complete page and checkpoint when a required EV recomputation request cannot be recorded.
...
- Remove DataForrest launch registration for aggregate catalog/pulls/sales pages, `has_more`, sale record kinds, and independent stream checkpoints in the same replacement.
...
- Run a test-only Courtyard source adapter with a different raw wrapper, checkpoint grammar, continuation signal, and separate connection profile through the same generic import path.
- Seed a distinct test-only Courtyard source instance at null through task 002's persistence contract, preserve its opaque checkpoint exactly across commit and resume, and retain the separate DataForrest source's history and checkpoint; task 004 owns activation lifecycle.
...
- Reject an identity-namespace mismatch instead of adding an ID crosswalk, checkpoint converter, dual read, or reconciliation ledger.
...
| Checkpoint | Requested and committed safe fingerprints, codec, and generation |
...
A failed attempt returns a stable retry or action-required class and cannot change the committed checkpoint.
