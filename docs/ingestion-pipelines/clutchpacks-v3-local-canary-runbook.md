# ClutchPacks V3 local canary qualification and replay

Status: guarded operator runbook for the first local V3 replay

This workflow qualifies and starts only the ClutchPacks adapter-v3 source in
the fresh, separate local PostgreSQL database created by
`bootstrap:clutchpacks-v3-canary:local`. It does not use Neon, clone the active
...
- Before every driver action, the active V1 source database must prove the
  exact buyback-EV-revisions and platform-request-lanes migration checksums and
  its intentional 87-application-table schema. The target independently proves
  the current 91-table composite schema through its full lifecycle startup.
...
- The target connection and source revisions use
  `dataforrest-events-adapter-v3` only. The source revision must also exactly
...
- The local target supervisor preserves an explicit 16 GiB free-space reserve.
  This value is specific to the isolated ClutchPacks canary and is not approval
  for the four-provider Task 010 historical stress forecast.
...
- Final target pause is allowed only after the actual latest, fully pinned V3
...
The 16 GiB local reserve is bound to measured Clutch-only evidence. The
completed V3 canary database is 518,806,551 bytes, while the committed capacity
artifact bounds one maximum page commit at 29,431,868 bytes and the ongoing
hook reserves four such turns (117,727,472 bytes). Sixteen GiB therefore
preserves more than 33 completed-canary database footprints and more than 145
four-turn reserves after already-admitted work. On 2026-08-27 the measured
database volume had 148,094,758,912 bytes available. If a later rehearsal no
longer fits those bounds, stop and refresh the evidence; do not lower the
reserve to make admission pass.
...
4. Advance one transition at a time with the printed `advance` confirmation.
   ...
   npm run advance:clutchpacks-v3-canary:local -- \
     ...
     --confirmation "ADVANCE CLUTCHPACKS V3 LOCAL <digest>"
   ```
...
5. At `ready_to_resume`, inspect `--status` once more. Confirm the original is
   ...
   ```bash
   npm run advance:clutchpacks-v3-canary:local -- \
     --resume --confirmation "RESUME CLUTCHPACKS V3 LOCAL <digest>"
   ```
...
6. Let replay continue until `--status` reports all of the following exact
   ...
   Pause only the V3 canary with the printed `pauseTarget` confirmation:

   ```bash
   npm run advance:clutchpacks-v3-canary:local -- \
     --pause-target \
     --confirmation "PAUSE CLUTCHPACKS V3 TARGET LOCAL <digest>"
   ```
...
- original V1 database is not the exact 87-table/two-checksum schema, or its
