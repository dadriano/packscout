...
Diagnose and fix a bug or small task in a single pass. This is the lightweight alternative to `/design-to-tasks` + `/build-from-tasks` — for issues that need a plan but don't need task files or full parallel build orchestration.

**Input:** User arguments contain the bug report or task description. If empty, ask the user to describe the issue.
...
- Lead the plan with the fix, not the investigation story: Issue and Root Cause in one line each, then the numbered Changes.
- Number every ordered sequence; one bounded change per item. Cap visible lists at five — group longer lists without dropping required items.
- Fill `Estimated effort` with a concrete time range (for example "20-30 minutes including verification"), never a vague size.
- When asking for plan confirmation, end with one question the user can answer in under two minutes.
- Lead the report with what now works and how it was proven. State failures matter-of-factly: cause, then fix. No preamble, no closing pleasantries.

ADHD shaping changes presentation, not rigor. Keep the verification anchor, acceptance-criteria evidence, phase gates, and the review watch. If an ADHD rule conflicts with them, keep the requirement and adapt its presentation.
...
**Estimated effort:** [concrete time range including verification]
...
1. **Create a worktree and branch.** Never modify the user's primary checkout. Create an isolated worktree, for example:

```bash
git worktree add .worktrees/quickfix-<short-slug> -b fix/<short-slug> HEAD
```

Run all edits from inside that worktree path.

2. **Implement the changes** described in the confirmed plan inside the worktree.
3. **Follow existing patterns.** Match the codebase's style, naming, and architecture. Read `AGENTS.md`, `CLAUDE.md`, or equivalent project guidance first.
...
3. **Run the build** if the project has a build step (`npm run build` or equivalent from project docs). Run this inside the worktree.
...
```bash
git worktree remove .worktrees/quickfix-<short-slug>
```
...
1. **Spawn a background watcher agent** (in Claude Code, the Agent tool with `run_in_background`; otherwise run the polling loop directly) whose only job is to watch the PR.
...
3. **Act on signals:**
   ...
   - New review comments or requested changes: surface them immediately, address them on the same branch (create a fresh worktree from the branch if the original was removed), rerun the Phase 4 verification set, push the fixes, and continue the watch.
...
- **Always commit, push, and open a PR** unless the user explicitly asks for a code-only stop point.
- **Always remove the worktree** after pushing and opening the PR.
