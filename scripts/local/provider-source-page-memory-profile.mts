import { dataforrestEventsV1SourceAdapterManifest, dataforrestCourtyardDistributedV2SourceAdapterManifest,
  DATAFORREST_COURTYARD_DISTRIBUTED_V2_MAXIMUM_JSON_NODES, providerSourceRecordsPerRequest } from "@packscout/contracts";
...
export function providerSourceMemoryProfile(args: readonly string[]) {
  // Exercise the largest configurable request, not the 500-row default used
  // by the backfill forecast. Thirteen envelope nodes plus 82 empty facts per
  // record put this 5,000-row witness just below the independent 480k-node cap.
  if (args.length === 0) return Object.freeze({ name: "maximum-request", manifest: dataforrestEventsV1SourceAdapterManifest,
    concurrentPages: 4, warmupPageCount: 12, trialCount: 5, pagesPerTrial: 20,
    recordsPerPage: providerSourceRecordsPerRequest.maximum, emptyObjectFactsPerRecord: 82,
