...
const WORKFLOW = "bootstrap_clutchpacks_v2_canary_tenant";
...
const PROVIDER_DISPLAY_NAME = "ClutchPacks V2 Canary";
const PROFILE_DISPLAY_NAME = "DataForrest V2 ClutchPacks Canary";
const ACTOR_KEY = "system:clutchpacks-v2-canary-bootstrap";
const CONFIRMATION_PREFIX = "BOOTSTRAP CLUTCHPACKS V2 LOCAL";
...
export class ClutchpacksV2CanaryBootstrapError extends Error {
  override readonly name = "ClutchpacksV2CanaryBootstrapError";
...
function deterministicUuid(organizationId: string, purpose: string): string {
  const bytes = createHash("sha256")
    .update("packscout.clutchpacks-v2-canary-local-identity.v1")
    .update("\0")
...
export interface ClutchpacksV2CanaryBootstrapEnvironment {
...
export function readClutchpacksV2CanaryBootstrapEnvironment(
  environment: NodeJS.ProcessEnv,
  ...
  const targetDigest = createHash("sha256").update([
    "packscout.clutchpacks-v2-canary-separate-local-target.v1",
...
export function parseClutchpacksV2CanaryBootstrapCommand(
...
export interface ClutchpacksV2ReplayCapacitySnapshot {
  readonly clutchSourceState: string;
  readonly clutchActiveRunCount: number;
}

export function assessClutchpacksV2ReplayCapacity(
  snapshot: ClutchpacksV2ReplayCapacitySnapshot,
): Readonly<{
  ready: boolean;
  reason:
    | "ready"
...
export interface ClutchpacksV2CanaryTargetSnapshot {
...
export function assertClutchpacksV2CanaryTargetIsSafe(
  snapshot: ClutchpacksV2CanaryTargetSnapshot,
  environment: Pick<
    ClutchpacksV2CanaryBootstrapEnvironment,
    | "targetOrganizationId"
    | "providerId"
...
interface OriginalConnectionEvidence {
  ...
  readonly capacity: ClutchpacksV2ReplayCapacitySnapshot;
}
...
async function readTargetSnapshot(
  database: PackscoutPrismaClient,
  environment: ClutchpacksV2CanaryBootstrapEnvironment,
): Promise<ClutchpacksV2CanaryTargetSnapshot> {
  const [
    organizationCount,
    organization,
    providers,
...
async function executeBootstrap(
  ...
  before: ClutchpacksV2CanaryTargetSnapshot,
...
export function safeClutchpacksV2CanaryBootstrapFailure(error: unknown) {
  return Object.freeze({
    ok: false as const,
    operation: WORKFLOW,
    code: error instanceof ClutchpacksV2CanaryBootstrapError
      ? error.code
      : "UNEXPECTED_BOOTSTRAP_FAILURE",
  });
}

export function clutchpacksV2CanaryBootstrapUsage(): string {
  return `Usage:
...
Required protected environment:
  ...
  PACKSCOUT_CLUTCHPACKS_V1_ORGANIZATION_ID
  PACKSCOUT_CLUTCHPACKS_V2_CANARY_ORGANIZATION_ID
  PACKSCOUT_CLUTCHPACKS_V2_TARGET_ACK=${TARGET_ACKNOWLEDGEMENT}
...
async function main(): Promise<void> {
  ...
  if (argv.length === 1 && argv[0] === "--help") {
    process.stdout.write(`${clutchpacksV2CanaryBootstrapUsage()}\n`);
    return;
  }
  const environment = readClutchpacksV2CanaryBootstrapEnvironment(process.env);
  const command = parseClutchpacksV2CanaryBootstrapCommand(
    argv,
  ...
  try {
    ...
    const capacity = assessClutchpacksV2ReplayCapacity(original.capacity);
    const before = await readTargetSnapshot(targetLifecycle.client, environment);
    assertClutchpacksV2CanaryTargetIsSafe(before, environment);
    if (!command.execute) {
    ...
    assertClutchpacksV2CanaryTargetIsSafe(after, environment);
    if (
      !after.organization ||
...
if (
  ...
  main().catch((error: unknown) => {
    process.stderr.write(`${JSON.stringify(
      safeClutchpacksV2CanaryBootstrapFailure(error),
    )}\n`);
    process.exitCode = 1;
  });
}
