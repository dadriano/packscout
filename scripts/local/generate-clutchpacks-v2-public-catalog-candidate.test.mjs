...
const {
  ...
  "./generate-clutchpacks-v2-public-catalog-candidate.mts",
  import.meta.url,
);
...
function environment(overrides = {}) {
  return {
    ...
    PACKSCOUT_DATABASE_URL:
      "postgresql://operator@127.0.0.1:5432/packscout_clutchpacks_v2_canary",
    PACKSCOUT_CLUTCHPACKS_V2_CANARY_ORGANIZATION_ID: ORGANIZATION_ID,
...
function evidence(overrides = {}) {
  ...
  return {
    ...
    nonInfoOperationalEventCount: 0,
...
test("environment and command bind one exact query-free loopback target", () => {
  ...
  assert.equal(parsed.databaseTarget,
    "postgresql://127.0.0.1:5432/packscout_clutchpacks_v2_canary");
  assert.deepEqual(
  ...
  assert.throws(
    () => readClutchpacksCatalogCandidateEnvironment(environment({
      PACKSCOUT_DATABASE_URL:
        "postgresql://operator@127.0.0.1:5432/packscout_clutchpacks_v2_canary?schema=public",
    })),
  ...
  await assert.rejects(
    runClutchpacksCatalogCandidate({
      ...
      argv: [
        ...
        "--confirmation", "WRITE CLUTCHPACKS V2 CATALOG LOCAL wrong",
      ],
    }),
