...
export class PhygitalsDataforrestActivationError extends Error {
  constructor(code) {
    super(code);
    this.name = "PhygitalsDataforrestActivationError";
    this.code = code;
  }
}
...
/**
 * Phygitals activation may only reuse the already-authorized central
 * DataForrest authority. A process/file token would bypass that lineage, so
 * remove it before failing closed.
 */
export function assertPhygitalsDataforrestTokenAbsent(environment) {
...
export function assertNoPhygitalsActivationArguments(argumentsList) {
  if (argumentsList.length !== 0) refuse("ACTIVATION_ARGUMENTS_FORBIDDEN");
}

export function readPhygitalsDataforrestActivationEnvironment(input) {
  if (input.processEnvironment.NODE_ENV !== "development") {
    refuse("LOCAL_DEVELOPMENT_ENVIRONMENT_REQUIRED");
  }
  assertPhygitalsDataforrestTokenAbsent(input.processEnvironment);
  assertPhygitalsDataforrestTokenAbsent(input.fileEnvironment);
...
export function phygitalsDataforrestConfiguration() {
  return Object.freeze({ platform: "phygitals" });
}

export function safePhygitalsDataforrestActivationError(error) {
  return error instanceof PhygitalsDataforrestActivationError
    ? error
    : new PhygitalsDataforrestActivationError(
        "PHYGITALS_DATAFORREST_ACTIVATION_FAILED",
      );
}
