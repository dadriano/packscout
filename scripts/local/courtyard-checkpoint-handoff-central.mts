...
export async function readCourtyardHandoffAuthority(client: CentralQueryClient, operationId: string) {
  ...
  if (provider.organization_id !== pins.organizationId || provider.provider_key !== pins.providerKey || provider.lifecycle !== "active" ||
    ...
    previous.version_number !== 1n || previous.adapter_key !== pins.previousAdapter || previous.endpoint_url !== DATAFORREST_EVENTS_V1_ENDPOINT ||
    ...
    ![pins.previousConfigId, nextConfigId].includes(provider.active_config_version_id ?? "") || versions.length !== (next ? 2 : 1) ||
    Boolean(stage) !== Boolean(next) || (next && (next.version_number !== 2n || next.adapter_key !== pins.nextAdapter ||
...
export async function stageCourtyardHandoff(input: Readonly<{ central: CentralPrismaClient; authority: CourtyardAuthority;
  ...
  if (input.checkpoint.runtimeState !== "paused" || input.checkpoint.generation !== "3" ||
  ...
  await input.central.$transaction(async (tx) => {
    ...
    await tx.provider_config_versions.create({ data: { id: current.nextConfigId, provider_id: current.provider.id,
      version_number: 2n, adapter_key: pins.nextAdapter, endpoint_url: current.previous.endpoint_url,
    ...
    await tx.provider_connection_tests.create({ data: { id: id(receipt.operationId, "activation-test"), provider_id: current.provider.id,
      ...
      record_counts: { sourceRecords: 100, canonicalValidCollectibles: proof.collectibleValidated,
        canonicalMissingDisplayNameRejected: proof.canonicalMissingDisplayNameRejected, mapperQuarantined: 0 },
    ...
    await tx.audit_events.create({ data: { id: id(receipt.operationId, "stage-audit"), organization_id: pins.organizationId,
      actor_key: "system:local-courtyard-checkpoint-handoff", action: `${pins.action}.staged`, subject_type: "provider",
...
export async function activateCourtyardHandoffLast(input: Readonly<{ central: CentralPrismaClient;
  ...
  if (checkpoint.runtimeState !== "paused" || checkpoint.generation !== "3" || checkpoint.cachedConfigId !== receipt.nextConfigId ||
    checkpoint.cachedConfigNumber !== "2" || checkpoint.cursorHash !== migrated.cursorHash || handoffDigest(checkpoint.cursor) !== handoffDigest(migrated.cursor) ||
  ...
  await input.central.$transaction(async (tx) => {
    ...
    await tx.audit_events.create({ data: { id: id(receipt.operationId, "activate-audit"), organization_id: pins.organizationId,
      actor_key: "system:local-courtyard-checkpoint-handoff", action: `${pins.action}.activated`, subject_type: "provider",
