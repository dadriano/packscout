...
import {
  ...
  phygitalsReplayPins as pins,
  probePhygitalsCardMapping,
  refusePhygitalsReplay as refuse,
  type PhygitalsReplaySnapshot,
} from "./prepare-phygitals-card-replay-plan.mts";

const replayAction = "provider.local_phygitals_card_replay_prepared";
const newAdapter = DATAFORREST_PHYGITALS_DISTRIBUTED_ADAPTER_VERSION;
...
async function centralSnapshot(client: CentralQueryClient) {
  ...
  if (
    ...
    provider.topology_version !== 2n || provider.row_version !== (phase === "previous" ? 4n : 5n) ||
    ...
    config.version_number !== (phase === "previous" ? 2n : 3n) ||
    config.adapter_key !== (phase === "previous" ? DATAFORREST_LAUNCH_DISTRIBUTED_ADAPTER_VERSION : newAdapter) ||
    ...
    previous.provider_id !== pins.providerId || previous.version_number !== 2n ||
    previous.adapter_key !== DATAFORREST_LAUNCH_DISTRIBUTED_ADAPTER_VERSION ||
    ...
    sourceCount !== 1 || configCount !== (phase === "previous" ? 2 : 3)
...
export async function readPhygitalsReplaySnapshot(database: ProviderPrismaClient): Promise<PhygitalsReplaySnapshot> {
  const [identity, runtime, run, activeRunCount, actionableCommandCount, runCount, commandCount,
    ...
    await Promise.all([
      ...
      database.quarantine_records.count({ where: {
        provider_run_id: pins.stoppedRunId, state: "expired",
...
async function activateRevision(input: Readonly<{
  ...
  sourceProof: Awaited<ReturnType<typeof probePhygitalsCardMapping>>;
  ...
  await input.central.$transaction(async (transaction) => {
    ...
    await transaction.provider_config_versions.create({ data: {
      id: pins.configId, provider_id: pins.providerId, version_number: 3n,
    ...
    await transaction.provider_connection_tests.create({ data: {
      ...
      result_summary: {
        ...
        replayOfRunId: pins.stoppedRunId, preservedQuarantineCount: 13_300,
      },
      record_counts: { records: 100, collectibles: 100, quarantined: 0 },
      tested_by_operator_id: locked.operatorId, tested_at: now, created_at: now,
    } });
    const updated = await transaction.providers.updateMany({
      where: { id: pins.providerId, row_version: 4n, active_config_version_id: pins.previousConfigId },
    ...
    await transaction.audit_events.create({ data: {
      ...
      actor_key: "system:local-phygitals-card-replay-v3", action: replayAction,
      ...
      metadata_json: {
        ...
        stoppedRunId: pins.stoppedRunId, preservedQuarantineCount: 13_300,
...
/** No automatic queue/run. Only the approved checkpoint can advance to config v3 at origin. */
export async function preparePhygitalsCardReplay(input: Readonly<{ apply: boolean }>) {
  ...
  try {
    ...
    const checkpoint = await gateway.runWithCachedProviderDatabase(route.route, readPhygitalsReplaySnapshot);
    if (checkpoint.state !== "reachable") return refuse("PHYGITALS_REPLAY_DATABASE_UNREACHABLE");
    const localPhase = assertPhygitalsReplaySnapshot(checkpoint.value);
    ...
    let sourceProof: Awaited<ReturnType<typeof probePhygitalsCardMapping>>;
    try { sourceProof = await probePhygitalsCardMapping(token); } finally { token = ""; }
    ...
    const preparation = await gateway.runWithCachedProviderDatabase(route.route, async (database) => {
      ...
      try {
        const frozen = await readPhygitalsReplaySnapshot(database);
        const phase = assertPhygitalsReplaySnapshot(frozen);
        if (phase !== localPhase) refuse("PHYGITALS_REPLAY_CHECKPOINT_CHANGED");
        const after = await executeGuardedPhygitalsReplay({
          readCheckpoint: () => readPhygitalsReplaySnapshot(database),
          ...
          synchronize: async () => {
            ...
            const synchronized = await new PrismaProviderRuntimeRepository(database).synchronizeConfiguration({
              ...
              configVersionId: pins.configId, configVersionNumber: 3n,
        ...
        if (phase === "previous") await database.local_audit_events.create({ data: {
          ...
          details: { previousConfigId: pins.previousConfigId, configId: pins.configId,
            ...
            historicalQuarantinesPreserved: 13_300, canonicalChangesBeforeReplay: 0 },
    ...
    return { outcome: "prepared", providerId: pins.providerId, configVersionId: after.config.id,
      configVersionNumber: 3, adapterKey: newAdapter, activationTestId: pins.activationTestId,
      preservedRunId: pins.stoppedRunId, preservedQuarantineCount: 13_300,
...
if (isMain) {
  ...
  if (argumentsList.length !== 1 || !["--verify", "--apply"].includes(argumentsList[0]!)) {
    ...
    preparePhygitalsCardReplay({ apply: argumentsList[0] === "--apply" })
      .then((result) => process.stdout.write(`${JSON.stringify(result)}\n`))
      .catch((error: unknown) => {
        console.error(JSON.stringify({ failureCode: error instanceof PhygitalsCardReplayError
