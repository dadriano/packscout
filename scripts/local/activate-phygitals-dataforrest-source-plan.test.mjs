...
import {
  ...
  assertPhygitalsDataforrestTokenAbsent,
...
function hasCode(code) {
  return (error) =>
    error instanceof PhygitalsDataforrestActivationError &&
...
test("activation rejects and removes any process or file bearer token", () => {
  ...
  assert.throws(
    () => assertPhygitalsDataforrestTokenAbsent(direct),
    hasCode("DATAFORREST_PROCESS_TOKEN_FORBIDDEN"),
  );
  assert.equal(direct.PACKSCOUT_DATA_API_TOKEN, undefined);
});

test("Phygitals source configuration is exact and immutable", () => {
  const configuration = phygitalsDataforrestConfiguration();
  assert.deepEqual(configuration, { platform: "phygitals" });
...
test("activation is local-only, argument-free, and safely errors", () => {
  ...
  assert.throws(
    () => assertNoPhygitalsActivationArguments(["--token", token]),
    hasCode("ACTIVATION_ARGUMENTS_FORBIDDEN"),
  );
  const safe = safePhygitalsDataforrestActivationError(
    new Error(`upstream included ${token}`),
  );
  assert.equal(safe.code, "PHYGITALS_DATAFORREST_ACTIVATION_FAILED");
