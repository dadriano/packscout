...
export function requireLoopbackConvexUrl(environment) {
  ...
  const origins = candidates.map((candidate) => {
    ...
    if (
      ...
      throw new Error(
        "Mock catalog tooling accepts only a root HTTP loopback Convex URL.",
      );
    }
...
export async function seedLocalMockCatalog() {
  ...
  await runNpx([
    "env",
    "set",
    "PACKSCOUT_MOCK_CATALOG_SEED_ENABLED",
    "1",
  ]);
  ...
  try {
    ...
    console.log(`Mock catalog ${result.status}; 9 packs are available locally.`);
    return result;
  } finally {
    try {
      await runNpx([
