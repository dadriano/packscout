...
import {
  PhygitalsDataforrestActivationError,
  assertNoPhygitalsActivationArguments,
  phygitalsDataforrestConfiguration,
  readPhygitalsDataforrestActivationEnvironment,
  safePhygitalsDataforrestActivationError,
} from "./activate-phygitals-dataforrest-source-plan.mjs";
import {
...
const PHYGITALS_PROVIDER_KEY = "phygitals";
const CLUTCHPACKS_PROVIDER_KEY = "clutchpacks";
const ACTIVATION_ACTOR =
  "system:local-phygitals-dataforrest-activation";
const ACTIVATION_ACTION = "provider.local_dataforrest_activation";

interface PhygitalsSnapshotRow extends QueryResultRow {
...
interface PhygitalsSnapshotCounts extends QueryResultRow {
...
interface PhygitalsSnapshot {
  readonly row: PhygitalsSnapshotRow;
  readonly counts: PhygitalsSnapshotCounts;
}
...
function refuse(code: string): never {
  throw new PhygitalsDataforrestActivationError(code);
}
...
export async function loadPhygitalsDataforrestRepositoryEnvironment():
Promise<Record<string, string>> {
  try {
    ...
    if (error instanceof PhygitalsDataforrestActivationError) throw error;
...
async function readPhygitalsSnapshot(
  client: SqlClient,
  providerId: string | null = null,
): Promise<PhygitalsSnapshot> {
  const result = await client.query<PhygitalsSnapshotRow>(`
  ...
  `, [PHYGITALS_PROVIDER_KEY, providerId]);
  const row = result.rows[0];
  if (result.rows.length !== 1 || row === undefined) {
    refuse("ACTIVATION_STATE_UNEXPECTED");
  }
  const countsResult = await client.query<PhygitalsSnapshotCounts>(`
    select
      ...
      (select count(*)::text from provider_connection_tests test
       where test.provider_id = $3::uuid
         ...
         and test.response_status is null and test.latency_ms is null
         and test.record_counts is null and test.has_more is null
         and test.next_cursor_present is null
  ...
  `, [
    ...
    JSON.stringify({
      ...
      platform: PHYGITALS_PROVIDER_KEY,
...
function exactPhygitalsBaseState(snapshot: PhygitalsSnapshot): boolean {
  const { row, counts } = snapshot;
  return row.provider_key === PHYGITALS_PROVIDER_KEY &&
    ...
    row.host === "127.0.0.1" && row.port === 55_435 &&
    row.database_name === "packscout_phygitals" &&
...
function exactPhygitalsBaselineState(snapshot: PhygitalsSnapshot): boolean {
  const { row, counts } = snapshot;
  return exactPhygitalsBaseState(snapshot) &&
    ...
    isExactObject(row.configuration, phygitalsDataforrestConfiguration()) &&
...
function exactPhygitalsActiveState(snapshot: PhygitalsSnapshot): boolean {
  const { row, counts } = snapshot;
  return exactPhygitalsBaseState(snapshot) &&
...
function phygitalsDatabasePins(
  snapshot: PhygitalsSnapshot,
): Readonly<ProviderReviewActivationDatabasePins> {
  if (!exactPhygitalsBaseState(snapshot)) {
    refuse("ACTIVATION_STATE_UNEXPECTED");
  }
  const { row } = snapshot;
  return Object.freeze({
...
async function readClutchpacksSourceAuthorityRow(input: Readonly<{
  ...
  if (
    ...
    row.activated_at.getTime() > Date.now() ||
...
export interface PhygitalsDataforrestActivationPlanningState {
  readonly state: "active" | "baseline" | "unexpected";
  readonly providerId: string;
  readonly providerKey: "phygitals";
  readonly configVersionNumber: number;
}

/** Reads only central metadata; it never decrypts or contacts DataForrest. */
export async function inspectPhygitalsDataforrestActivationPlanningState(
  pool: Pool,
): Promise<PhygitalsDataforrestActivationPlanningState> {
  const snapshot = await readPhygitalsSnapshot(pool);
  return Object.freeze({
    state: exactPhygitalsActiveState(snapshot)
      ? "active"
      : exactPhygitalsBaselineState(snapshot)
        ? "baseline"
        : "unexpected",
    providerId: snapshot.row.provider_id,
    providerKey: PHYGITALS_PROVIDER_KEY,
    configVersionNumber: Number(snapshot.row.config_version_number),
  });
}

async function activatePhygitals(input: Readonly<{
  pool: Pool;
  baseline: PhygitalsSnapshot;
  cipher: AesGcmProviderCredentialCipher;
  ...
  if (
    ...
    input.databaseProof.runCount !== 0 ||
    input.databaseProof.commandCount !== 0 ||
    input.databaseProof.canonicalCount !== 0 ||
    input.databaseProof.quarantineCount !== 0
  ) {
  ...
  try {
    ...
    const lockedBaseline = await readPhygitalsSnapshot(
      client,
      input.baseline.row.provider_id,
    );
    if (
      !exactPhygitalsBaselineState(lockedBaseline) ||
    ...
    `, [
      ...
      DATAFORREST_EVENTS_V1_ENDPOINT,
      sourceCredentialVersionId,
      JSON.stringify(phygitalsDataforrestConfiguration()),
...
export async function runPhygitalsDataforrestActivationCli(input: Readonly<{
  ...
  let environment: ReturnType<
    typeof readPhygitalsDataforrestActivationEnvironment
  > | null = null;
  let central: Pool | null = null;
  try {
    fileEnvironment =
      await loadPhygitalsDataforrestRepositoryEnvironment();
    environment = readPhygitalsDataforrestActivationEnvironment({
      processEnvironment,
      fileEnvironment,
    });
    central = new Pool({
    ...
    let snapshot = await readPhygitalsSnapshot(central);
    const alreadyActive = exactPhygitalsActiveState(snapshot);
    if (!alreadyActive && !exactPhygitalsBaselineState(snapshot)) {
      refuse("ACTIVATION_STATE_UNEXPECTED");
    }
    const databaseProof = await runPinnedProviderReviewActivationDatabaseProof({
      ...
      cipher,
      pins: phygitalsDatabasePins(snapshot),
    ...
    if (alreadyActive) {
      ...
      const activation = await activatePhygitals({
        pool: central,
        baseline: snapshot,
        cipher,
        databaseProof,
      ...
      snapshot = await readPhygitalsSnapshot(
        central,
        snapshot.row.provider_id,
      );
      if (
        !exactPhygitalsActiveState(snapshot) ||
...
if (isMain) {
  runPhygitalsDataforrestActivationCli().catch((error: unknown) => {
    const safe = safePhygitalsDataforrestActivationError(error);
    console.error(JSON.stringify({
      level: "error",
      event: "phygitals_dataforrest_activation_failed",
