...
import { CourtyardHandoffError, courtyardHandoff as pins, courtyardHandoffId as id, refuseCourtyardHandoff as refuse,
  ...
  type CourtyardCheckpoint, type CourtyardReceipt, type CourtyardCanaryProof } from "./courtyard-checkpoint-handoff-plan.mts";
import { readCourtyardHandoffAuthority, stageCourtyardHandoff, activateCourtyardHandoffLast,
  type CourtyardAuthority } from "./courtyard-checkpoint-handoff-central.mts";
import { courtyardTerminalReceipt, readCourtyardReceipt, pauseCourtyardTerminal,
  assertCourtyardPauseProvenance, resumeCourtyardHandoff } from "./courtyard-checkpoint-handoff-control.mts";
import { probeCourtyardHandoff } from "./courtyard-checkpoint-handoff-canary.mts";

export { CourtyardHandoffError };
const allowedCourtyardHandoffCodes = new Set([
  "COURTYARD_CANARY_ADMISSION_FAILED", "COURTYARD_CANARY_AUTHORITY_INVALID", "COURTYARD_CANARY_COLLECTIBLE_REJECTED",
...
export function parseCourtyardHandoffArguments(args: readonly string[]) {
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/u.test(operationId)) refuse("COURTYARD_OPERATION_ID_INVALID");
...
export async function runCourtyardHandoff(args: ReturnType<typeof parseCourtyardHandoffArguments>) {
  // Shared bootstrap only: this loader rejects bearer overrides and admits only central55431 + cipher keyring.
  ...
  try {
    ...
    const result = await gateway.runWithCachedProviderDatabase(route.route, async (database) => captureCourtyardHandoffResult(async () => {
      ...
      const owner = `local:courtyard:handoff:${args.operationId}`;
      ...
      const reclaim = staged ? { reclaimableOwner: owner } : {};
      ...
      if (args.mode === "--resume") {
        ...
        return resumeCourtyardHandoff({ database, receipt, cursorHash: migrated.cursorHash, assertPrepared: async (resumed) => {
          const current = await readCourtyardHandoffCheckpoint(database);
          if (current.runtimeState !== (resumed ? "idle" : "paused") || current.generation !== (resumed ? "4" : "3")) refuse("COURTYARD_RESUME_STATE_CHANGED");
          assertCourtyardCheckpoint({ snapshot: { ...current, runtimeState: "paused", generation: "3" },
      ...
      await stageCourtyardHandoff({ central: central.client, authority, receipt, checkpoint: fresh, sourceProof });
      ...
      const locked = async (action: (tx: ProviderTransactionClient, checkpoint: CourtyardCheckpoint) => Promise<void>) => database.$transaction(async (tx) => {
        const lease = await lockProviderWorkerLease(tx, "import");
        if (!providerWorkerLeaseIsLive(lease, { owner, fence: acquired.lease.fence })) refuse("COURTYARD_UTILITY_LEASE_EXPIRED");
      ...
      try {
        await locked(async (tx, current) => {
          ...
          const updated = await tx.provider_runtime.updateMany({ where: { singleton_key: true, operating_state: "paused", state_generation: 3n,
            ...
            cached_config_version_id: receipt.nextConfigId, cached_config_version_number: 2n,
