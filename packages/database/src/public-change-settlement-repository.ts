...
export async function allocatePublicChangeCauses(
  ...
  const preparedCauses = input.changes.map((change, index) => ({
  ...
  const rows = preparedCauses.map((cause) => Prisma.sql`(
    ...
    ${cause.occurredAt}, pg_current_xact_id()::text, ${cause.occurredAt}
  )`);
  const transactionRows = await database.$queryRaw<
  ...
  >(Prisma.sql`
    ...
    returning authoritative_transaction_id as "transactionId"
  `);
  const transactionId = transactionRows[0]?.transactionId;
  if (
    !transactionId
    || transactionRows.length !== preparedCauses.length
    || transactionRows.some((row) => row.transactionId !== transactionId)
  ) {
    throw new Error("Public change transaction identity failed.");
  }
  const causes: AllocatedPublicChangeCause[] = preparedCauses.map((cause) => ({
    ...cause,
...
export async function createPublicDerivationObligationBatch(
  ...
  if (input.obligations.length > maximumRowsPerWrite) {
    throw new RangeError("Public derivation obligation batch is too large.");
  }
  for (const obligation of input.obligations) {
    ...
    if (!Number.isFinite(obligation.createdAt.getTime())) {
      throw new RangeError("Public derivation creation time is invalid.");
    }
  }
  ...
  for (const obligation of input.obligations) {
    for (const causeSequence of new Set(obligation.causeSequences)) {
      rowsByIdentity.set(
        [causeSequence, obligation.derivationKind, obligation.derivationKey].join(
          "\u0000",
        ),
        {
          causeSequence,
  ...
  const obligationRows = [...rowsByIdentity.values()];
  if (obligationRows.length === 0) return;
  const uniqueSequences = [...new Set(
    obligationRows.map(({ causeSequence }) => causeSequence),
  )].sort((left, right) =>
  ...
  for (const sequenceBatch of batches(uniqueSequences)) {
    await assertCatalogObligationsAheadOfSettlement(database, {
      organizationId: input.organizationId,
      causeSequences: sequenceBatch,
    });
  }
  for (const rowBatch of batches(obligationRows)) {
    const rows = rowBatch.map((obligation) => Prisma.sql`(
      ${uuid(input.organizationId)}, ${obligation.causeSequence},
      cast(${obligation.derivationKind} as public.public_derivation_kind),
      ${obligation.derivationKey}, ${obligation.createdAt}, ${obligation.createdAt}
    )`);
    await database.$executeRaw(Prisma.sql`
