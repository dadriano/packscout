...
export interface ApprovedPublicCatalogConfigurationRecord {
  ...
  readonly publicChangeSequence: bigint;
  /**
   * Explicit bounded follow-up. Run the Heat rematerialization repository with
   * this sequence after the approval transaction commits; never replay inside
   * this governed configuration transaction.
   */
  readonly heatRematerialization: Readonly<{
  ...
  async approveConfiguration(
    ...
    return this.database.$transaction(async (transaction) => {
      ...
      return {
        ...
        configurationHash,
        publicChangeSequence: cause.sequence,
        heatRematerialization: materializedMappings.created === 0
          ? null
          : {
              mappingPublicChangeSequence: cause.sequence,
              createdMappingCount: materializedMappings.created,
            },
      };
