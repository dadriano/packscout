...
import { PrismaNormalizedHeatRematerializationRepository } from "./normalized-heat-rematerialization-repository.ts";
import {
...
const ids = {
  ...
  secondMissingMappingRun: "55000000-0000-4000-8000-00000000003a",
...
function catalogPayload(
  ...
  return {
    ...
    collected_at: occurredAt.toISOString(),
    available: true,
    data,
  };
}
    ...
    records: [
      {
        ...
        sourceTime: sourceAt,
        collectedAt,
        payload: catalogPayload("catalog-page-1", sourceAt, {
          protected: "private-catalog",
        }),
        projections: [
          {
      ...
      },
      {
        ...
        sourceTime: sourceAt,
        collectedAt,
        payload: pullPayload("pull-valued", "pack-1", sourceAt, {
          protected: "private-valued-actor",
        }),
        projections: [{
          platformKey: "platform-a",
          recordKind: "pull",
      ...
      },
      {
        ...
        sourceTime: sourceAt,
        collectedAt,
        payload: pullPayload("pull-missing-value", "pack-1", sourceAt, {
          protected: "private-null-actor",
        }),
        projections: [{
          platformKey: "platform-a",
          recordKind: "pull",
...
async function commitExactArithmeticEvidence(input: {
  ...
  await input.ingestion.commitPage({
    ...
    pageNumber: 1,
    requestedCursor: null,
    nextCursor: `cursor-${ids.exactArithmeticRun}`,
    hasMore: false,
    checkpointMode: "provider",
    ...
    records: [
      {
        ...
        collectedAt: input.occurredAt,
        payload: catalogPayload(
          "catalog-exact-arithmetic",
          input.occurredAt,
          { protected: "pack-price" },
        ),
        projections: [{
          platformKey: "platform-a",
          recordKind: "pack",
...
test("canonical writes persist settled, bounded, public-safe Heat observations without replay duplication", async () => {
  ...
  try {
    ...
    await commitSinglePull({
      ingestion,
      setup,
      runId: ids.secondMissingMappingRun,
      externalId: "pull-unmapped-second",
      packExternalId: "unmapped-pack",
      occurredAt: new Date(sourceAt.getTime() + 2),
      value: { amountMinor: 3_000, currency: "USD" },
    });
    assert.equal(await harness.client.normalized_heat_observations.count(), 5);
    assert.deepEqual(
      (
    ...
    await harness.client.$transaction(async (transaction) => {
      ...
      lateMappingSequence = causes[0]!.sequence;
    ...
    assert.ok(lateMappingSequence > 0n);
    const rawExpiredAt = new Date(sourceAt.getTime() + 3);
    await harness.client.$executeRaw`
      update source_records
      set payload_json = null,
          payload_expired_at = ${rawExpiredAt}
      where organization_id = ${ids.organization}::uuid
        and external_id in ('pull-unmapped', 'pull-unmapped-second')
    `;
    await harness.client.$executeRaw`
      update import_pages
      set payload_json = null,
      ...
      where organization_id = ${ids.organization}::uuid
        and run_id in (
          ${ids.missingMappingRun}::uuid,
          ${ids.secondMissingMappingRun}::uuid
        )
    `;
    const expiredRawRecords = await harness.client.$queryRaw<Array<{
      count: bigint;
    }>>`
    ...
    `;
    assert.equal(
      expiredRawRecords[0]?.count,
      2n,
      "the replay fixture must have no retained provider record payload",
    );

    const firstReplayClient = await harness.createIndependentClient();
    const secondReplayClient = await harness.createIndependentClient();
    const replayedAt = new Date(sourceAt.getTime() + 4);
    const concurrentReplayResults = await Promise.all([
      new PrismaNormalizedHeatRematerializationRepository(firstReplayClient, {
        organizationId: ids.organization,
      }).replayMissingMappingsBatch({
        mappingPublicChangeSequence: lateMappingSequence,
        replayedAt,
        limit: 1,
      }),
      new PrismaNormalizedHeatRematerializationRepository(secondReplayClient, {
        organizationId: ids.organization,
      }).replayMissingMappingsBatch({
        mappingPublicChangeSequence: lateMappingSequence,
        replayedAt,
    ...
    const concurrentlyClaimed = concurrentReplayResults.flatMap(
      ({ claimedSourceCandidateKeys }) => claimedSourceCandidateKeys,
    );
    assert.equal(concurrentlyClaimed.length, 2);
    assert.equal(new Set(concurrentlyClaimed).size, 2);
    assert.equal(
      concurrentReplayResults.reduce((total, result) => total + result.normalized, 0),
      2,
    );
    assert.equal(
      concurrentReplayResults.reduce((total, result) => total + result.eligible, 0),
      2,
    );
    assert.equal(
      concurrentReplayResults.reduce(
        (total, result) => total + result.retentionExpired,
        0,
      ),
      0,
    );

    const rematerializer = new PrismaNormalizedHeatRematerializationRepository(
      harness.client,
      { organizationId: ids.organization },
    );
    assert.deepEqual(
      await rematerializer.replayMissingMappingsBatch({
        mappingPublicChangeSequence: lateMappingSequence,
        replayedAt,
        limit: 10,
      }),
      {
        ...
        retentionExpired: 0,
        claimed: 0,
        claimedSourceCandidateKeys: [],
        nextCursor: null,
        scanExhausted: true,
      },
      "a completed replay sweep is idempotent",
    );
    await assert.rejects(
      new PrismaNormalizedHeatRematerializationRepository(harness.client, {
        organizationId: ids.otherOrganization,
      }).replayMissingMappingsBatch({
        mappingPublicChangeSequence: lateMappingSequence,
        replayedAt,
    ...
    const rematerialized =
      await harness.client.normalized_heat_observations.findMany({
        where: {
          organization_id: ids.organization,
          public_repack_id: ids.secondPublicRepack,
          occurred_at: new Date(sourceAt.getTime() + 2),
        },
        orderBy: { public_change_sequence: "asc" },
      });
    assert.equal(rematerialized.length, 2);
    assert.ok(rematerialized.every(({ mapping_public_change_sequence }) =>
      mapping_public_change_sequence === lateMappingSequence));
    assert.ok(rematerialized.every(({ public_change_sequence }) =>
      public_change_sequence < lateMappingSequence));
    assert.ok(rematerialized.every(({ retained_until }) =>
      retained_until.toISOString() === "2026-08-22T10:01:00.002Z"));
    assert.doesNotMatch(
      JSON.stringify(
        rematerialized,
        (_key, value: unknown) => typeof value === "bigint" ? value.toString() : value,
      ),
      /wallet|actor|payload|private/i,
    );
    assert.equal(
      await harness.client.normalized_heat_observation_outcomes.count({
        where: {
          ...
          canonical_revision_id: {
            in: rematerialized.map(({ canonical_revision_id }) =>
              canonical_revision_id),
          },
          status: "deferred",
      ...
      2,
      "the original append-only deferred audit remains intact",
    );
    assert.equal(
      await harness.client.normalized_heat_observation_outcomes.count({
        where: {
          organization_id: ids.organization,
          observation_id: { in: rematerialized.map(({ id }) => id) },
          status: "normalized",
          reason_code: "NORMALIZED",
          mapping_public_change_sequence: lateMappingSequence,
        },
      ...
      2,
      "rematerialization appends one exact terminal outcome per observation",
    );
    ...
    assert.equal(await harness.client.normalized_heat_observations.count(), 8);
    const newlyMapped = await reader.listSettledNormalizedHeatObservations({
      ...
      causalSequenceLte: lateMappingSequence + 1n,
      limit: 100,
    });
    ...
    assert.equal(immutableHistory.length, 15);
