...
export interface DeferredNormalizedHeatRematerializationCandidate {
  readonly sourceCandidateKey: string;
  readonly revision: CanonicalHeatSourceRevision;
}

export interface NormalizedHeatRematerializationPersistenceResult
  extends NormalizedHeatPersistenceResult {
  /** Deferred candidates whose exact approved mapping belongs to this approval. */
  readonly eligible: number;
  /** Claimed rows for a different pack candidate on the same canonical revision. */
  readonly unrelated: number;
  /** Candidates outside the seven-day event-time retention window. */
  readonly retentionExpired: number;
}

export class NormalizedHeatRematerializationEvidenceError extends Error {
  readonly code = "NORMALIZED_HEAT_REMATERIALIZATION_EVIDENCE_INSUFFICIENT" as const;

  constructor() {
    super(
      "Canonical Heat evidence cannot reconstruct the deferred normalization candidate.",
    );
    this.name = "NormalizedHeatRematerializationEvidenceError";
  }
}
...
export async function persistNormalizedHeatObservationsForCanonicalWrites(
  ...
  for (const observationBatch of batches(observations)) {
    const values = observationBatch.map((observation) => Prisma.sql`(
  ...
  for (const outcomeBatch of batches(outcomes)) {
    const values = outcomeBatch.map((outcome) => Prisma.sql`(
...
interface PreparedRematerializationCandidate extends PreparedCandidate {
  readonly sourceCandidateKey: string;
  readonly mapping: MappingRow;
}

function rematerializationOutcomeKey(
  sourceCandidateKey: string,
  mappingPublicChangeSequence: bigint,
): string {
  return stablePublicEvidenceKey("normalization:outcome", [
    "mapping-rematerialization-v1",
    sourceCandidateKey,
    mappingPublicChangeSequence.toString(),
  ]);
}

function preparedRematerializationOutcome(input: {
  candidate: PreparedRematerializationCandidate;
  status: PreparedOutcome["status"];
  reasonCode: NormalizedHeatOutcomeReason;
  observationId?: string | null;
}): PreparedOutcome {
  return {
    ...
    candidateKey: rematerializationOutcomeKey(
      input.candidate.sourceCandidateKey,
      input.candidate.mapping.publicChangeSequence,
    ),
    packExternalId: input.candidate.packExternalId,
    mapping: input.candidate.mapping,
    status: input.status,
    reasonCode: input.reasonCode,
    observationId: input.observationId ?? null,
  };
...
/**
 * Converts a previously deferred mapping-missing candidate using only immutable
 * canonical evidence. The original deferred outcome remains untouched; this
 * writes a deterministic second outcome that records the later mapping cause.
 */
export async function persistRematerializedNormalizedHeatObservations(
  database: PackscoutTransactionClient,
  input: {
    organizationId: string;
    mappingPublicChangeSequence: bigint;
    candidates: readonly DeferredNormalizedHeatRematerializationCandidate[];
    replayedAt: Date;
  },
): Promise<NormalizedHeatRematerializationPersistenceResult> {
  assertTransactionClient(database);
  const organizationId = requireUuid(input.organizationId, "organizationId");
  ...
  if (input.candidates.length > maximumWriteCandidates) {
    throw new RangeError("Heat rematerialization exceeds its transaction bound.");
  }
  const sourceCandidateKeys = new Set<string>();
  for (const candidate of input.candidates) {
    if (!/^[0-9a-f]{64}$/.test(candidate.sourceCandidateKey)) {
      throw new RangeError("sourceCandidateKey is invalid.");
    }
    if (sourceCandidateKeys.has(candidate.sourceCandidateKey)) {
      throw new RangeError("Heat rematerialization candidates are not unique.");
    }
    sourceCandidateKeys.add(candidate.sourceCandidateKey);
    requireUuid(candidate.revision.revisionId, "revisionId");
    requireUuid(candidate.revision.entityId, "entityId");
    requireCanonicalDate(candidate.revision.occurredAt, "occurredAt");
    if (candidate.revision.publicChangeSequence < 1n) {
      throw new RangeError("Canonical Heat source sequence is invalid.");
    }
  }
  ...
  const revisionsById = new Map(
    input.candidates.map(({ revision }) => [revision.revisionId, revision]),
  );
  const classifiedByRevision = new Map<string, PreparedCandidate[]>();
  for (const revision of revisionsById.values()) {
    const classification = classifyHeatCandidate(revision);
    if (classification?.candidate) {
      classifiedByRevision.set(revision.revisionId, [classification.candidate]);
    }
  }
  const priorCatalogPacks = await loadPriorCatalogPackExternalIds(
    database,
  ...
  for (const revision of revisionsById.values()) {
    const previousPackExternalId = priorCatalogPacks.get(revision.revisionId);
    if (!previousPackExternalId || revision.recordKind !== "catalog_asset") {
      continue;
    }
    const values = classifiedByRevision.get(revision.revisionId) ?? [];
    if (!values.some(({ packExternalId }) =>
      packExternalId === previousPackExternalId)) {
      values.push({
        revision,
        kind: "catalog_snapshot",
    ...
    classifiedByRevision.set(revision.revisionId, values);
  }

  const exactCandidates: Array<PreparedCandidate & {
    sourceCandidateKey: string;
  }> = [];
  for (const requested of input.candidates) {
    const exact = (classifiedByRevision.get(requested.revision.revisionId) ?? [])
      .find((candidate) => candidateKey(candidate) === requested.sourceCandidateKey);
    if (!exact) throw new NormalizedHeatRematerializationEvidenceError();
    exactCandidates.push({ ...exact, sourceCandidateKey: requested.sourceCandidateKey });
  }
  const mappings = await loadMappings(database, organizationId, exactCandidates);
  const eligibleCandidates: PreparedRematerializationCandidate[] = [];
  let unrelated = 0;
  for (const candidate of exactCandidates) {
    ...
    if (
      !mapping
      || mapping.publicChangeSequence !== input.mappingPublicChangeSequence
    ) {
      unrelated += 1;
      continue;
    }
    eligibleCandidates.push({ ...candidate, mapping });
  }

  let retentionExpired = 0;
  let activeCandidates = eligibleCandidates.filter((candidate) => {
    if (retainedUntil(candidate.revision.occurredAt) <= input.replayedAt) {
      retentionExpired += 1;
      return false;
    }
    return true;
  ...
  await database.$executeRaw(Prisma.sql`
    insert into public.normalized_heat_window_checkpoints (organization_id, updated_at)
    values (${uuid(organizationId)}, ${input.replayedAt})
    on conflict (organization_id) do nothing
  `);
  const checkpoints = await database.$queryRaw<Array<{
    closedBefore: Date | null;
  ...
  const closedBefore = checkpoints[0]?.closedBefore ?? null;
  if (closedBefore) {
    const open: PreparedRematerializationCandidate[] = [];
    for (const candidate of activeCandidates) {
      if (candidate.revision.occurredAt < closedBefore) {
        outcomes.push(preparedRematerializationOutcome({
          candidate,
          status: "rejected",
          reasonCode: "WINDOW_CLOSED",
        }));
  ...
  const replayKeys = [
    ...
    ...activeCandidates.map((candidate) => rematerializationOutcomeKey(
      candidate.sourceCandidateKey,
      candidate.mapping.publicChangeSequence,
    )),
  ];
  const existingReplayKeys = new Set<string>();
  if (replayKeys.length > 0) {
    const existing = await database.$queryRaw<Array<{ candidateKey: string }>>(
      Prisma.sql`
        ...
        from public.normalized_heat_observation_outcomes
        where organization_id = ${uuid(organizationId)}
          and candidate_key in (${Prisma.join(replayKeys)})
      `,
    );
    existing.forEach(({ candidateKey: key }) => existingReplayKeys.add(key));
  }
  outcomes = outcomes.filter(({ candidateKey: key }) =>
    !existingReplayKeys.has(key));
  activeCandidates = activeCandidates.filter((candidate) =>
    !existingReplayKeys.has(rematerializationOutcomeKey(
      candidate.sourceCandidateKey,
      candidate.mapping.publicChangeSequence,
    )));
  ...
  let observations: Array<PreparedObservation & {
    sourceCandidateKey: string;
  }> = [];
  for (const candidate of activeCandidates) {
    if (candidate.kind === "pull") {
  ...
  for (const observationBatch of batches(observations)) {
    const values = observationBatch.map((observation) => Prisma.sql`(
      ...
      ${observation.valueMultipleBasisPoints},
      ${observation.availableChaseCount}, ${textArray(observation.outcomeKeys)},
      ${retainedUntil(observation.revision.occurredAt)}, ${input.replayedAt}
    )`);
    const inserted = await database.$queryRaw<
      Array<{ observationId: string; observationKey: string }>
    >(Prisma.sql`
  ...
  for (const outcomeBatch of batches(outcomes)) {
    const values = outcomeBatch.map((outcome) => Prisma.sql`(
      ...
      ${outcome.status}, ${outcome.reasonCode},
      ${outcome.observationId ? uuid(outcome.observationId) : Prisma.sql`null::uuid`},
      ${retainedUntil(outcome.revision.occurredAt)}, ${input.replayedAt}
    )`);
    await database.$executeRaw(Prisma.sql`
      insert into public.normalized_heat_observation_outcomes (
        organization_id, candidate_key, canonical_revision_id,
  ...
  const counts = outcomes.reduce<NormalizedHeatPersistenceResult>(
    (result, outcome) => ({
      ...result,
    ...
    }),
    {
      ...
      deferred: 0,
      rejected: 0,
      duplicate: existingReplayKeys.size,
    },
  );
  return {
    ...counts,
