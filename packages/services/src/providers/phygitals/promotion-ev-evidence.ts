...
const BUCKET_HOMOGENEITY_HASH_DOMAIN =
  "packscout.phygitals.distributed-v4.bucket-homogeneity.v1";
...
/**
 * Binds the reviewed distributed-v4 rarity distribution to the exact pack row
 * being promoted. Rarity buckets retain their provider-published probability
 * and value range; no inventory quantity or outcome value is invented.
 */
export async function normalizePhygitalsPromotionEvEvidenceV1(input: Readonly<{
  ...
  if (!parsed.success) throw new PhygitalsPromotionEvEvidenceError("EVIDENCE_INVALID");
  ...
  if (
    ...
    evidence.providerKey !== "phygitals" ||
    evidence.sourceTypeKey !== "dataforrest-events-v1" ||
    evidence.sourceAdapterVersion !== DATAFORREST_PHYGITALS_DISTRIBUTED_ADAPTER_V4_VERSION ||
    evidence.mapperKey !== "phygitals-provider-observation" ||
    ...
    throw new PhygitalsPromotionEvEvidenceError("EVIDENCE_SNAPSHOT_MISMATCH");
  }
  ...
  const collectionGuardSha256 = await sha256CanonicalJson(
    "packscout.phygitals.distributed-promotion-ev-guard.v1",
    {
  ...
  const outcomes: PackScoutBuybackEvOutcomeClaimV1[] = await Promise.all(
    (evInput?.buckets ?? []).map(async (bucket, index) => {
      ...
      const homogeneityEvidenceSha256 = await sha256CanonicalJson(
        BUCKET_HOMOGENEITY_HASH_DOMAIN,
        {
          evidenceHash,
          product,
          bucket: {
  ...
  const publishedEntries = (evInput?.buckets ?? []).flatMap((bucket, index) => {
    const probability = packScoutBuybackEvProbabilityFromRatioNumberV1(
      bucket.probability,
    );
  ...
  return finalizePackScoutBuybackEvEvidenceV1(
    {
      ...
      odds: {
        poolKind: "finite",
        currentPool: null,
        published: publishedEntries.length === 0
          ? null
          : {
