...
test("the production activation alert target is fixed at exactly one minute", () => {
  assert.throws(
    () => new PromotionOperationalReadinessService(
      {
        promotionActivationDelayed: async () => ({ status: "accepted", alertId: null, failureCode: null }),
        promotionSettlementBlocked: async () => ({ status: "accepted", alertId: null, failureCode: null }),
        promotionFailed: async () => ({ status: "accepted", alertId: null, failureCode: null }),
        promotionRecovered: async () => ({ status: "accepted", alertId: null, failureCode: null }),
      },
