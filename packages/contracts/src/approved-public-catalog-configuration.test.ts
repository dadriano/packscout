...
test("approved public configuration permits an empty governed repack catalog", () => {
  const input = configuration();
  input.repacks = [];

  const parsed = approvedPublicCatalogConfigurationV1Schema.parse(input);
  assert.deepEqual(parsed.repacks, []);
});
